...
var rooms = {
  default: {
    ...
    namesUsed: [],
    players: {},
    roleQueue: ['wereWolf', 'minion', 'mason', 'seer', 'robber', 'troubleMaker', 'drunk', 'insomniac']
  }
}
...
function handleNewPlayer(socket, rooms) {
  socket.on('join', function (message) {
    ...
    if (currentRoom.playerNumber < currentRoom.playerLimit) {
      ...
      currentRoom.playersQueue.push(socket.id)
...
function handleClientDisconnection(socket, rooms) {
  socket.on('disconnecting', function () {
    ...
    if (room && room.players[socket.id]) {
      ...
      room.playerNumber --
      let index = room.playersQueue.indexOf(socket.id)
      room.playersQueue.splice(index, 1)
